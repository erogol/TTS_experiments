...
def gaussian_loss(y_hat, quit, log_std_min=-7.0):
    assert y_hat.dim() == 3
...
def sample_from_gaussian(y_hat, log_std_min=-7.0, scale_factor=1.0):
    ...
    sample = dist.sample()
    sample = torch.clamp(torch.clamp(sample, min=-scale_factor),
                         max=scale_factor)
    del dist
    return sample
...
def discretized_mix_logistic_loss(y_hat,
                                  ...
                                  log_scale_min=None,
                                  reduce=True):
    """Discritized Mixture of Logistic Loss
    Args:
        y_hat (tensor): model predictions.
        y (tensor): labels.
        num_classes (int): number of classes.
        log_scale_min (float): min log value to prevent log(0).
        reduce (bool): if true, returns mean loss.

    Shapes:
        y_hat: B x T X 3 * (num_mix)
        y: B x T
    """
